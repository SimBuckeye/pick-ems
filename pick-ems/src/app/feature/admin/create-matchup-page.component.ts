...
export default class CreateMatchupPageComponent implements OnInit {
    ...
    rounds: WritableSignal<any[]> = signal([]);
    roundOptions: Signal<any[]> = computed(() =>
        this.rounds().map(r => ({ label: r.year + ' - ' + r.name, value: r.id })));
    loading: WritableSignal<boolean> = signal(true);
    selectedAwayTeam: WritableSignal<number | null> = signal(null);
    selectedHomeTeam: WritableSignal<number | null> = signal(null);
    selectedDate: WritableSignal<Date | null> = signal(new Date());
    selectedUnderdog: WritableSignal<string | null> = signal(null);
    selectedRound: WritableSignal<number | null> = signal(null);
    ...
    async onLoad() {
        ...
        let { data: teamsData, error: teamsError } = await this.supabase.from('team').select("id,name");
        ...
        let { data: roundsData, error: roundsError } = await this.supabase.from('round').select('id, year, name').eq('state', 'not_ready').order('id', { ascending: true });
